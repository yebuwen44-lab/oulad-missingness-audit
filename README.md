...
> Package version: v1.2. This release fixes Windows PowerShell 5.1 compatibility in the Python-free integrity verifier; research data and frozen numerical results are unchanged.
...
On Windows, this check uses the built-in Windows PowerShell and does not require Python. The original Python verifier is retained as an alternative command-line implementation. Both verify the processed-table dimensions, prevalence, Week-4 cohort, group count, feature counts, frozen AUPRC values, reconstructed LOGO macro values, and SHA-256 hashes.
...
- `results/rerun/` is intentionally empty at release and receives newly generated outputs.
